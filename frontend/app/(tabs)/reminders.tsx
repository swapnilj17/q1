...
export default function RemindersScreen() {
  ...
  const handleAddReminder = async () => {
    ...
    Keyboard.dismiss();
    try {
      const created = await api.createReminder({ title: newReminder.trim() });
  ...
  const handleClearCompleted = () => {
    ...
    if (completedCount === 0) {
      Alert.alert('Info', 'No completed reminders to clear');
      return;
    }

    Alert.alert(
      'Clear Completed',
      `Are you sure you want to delete ${completedCount} completed reminder${completedCount > 1 ? 's' : ''}?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await api.clearCompletedReminders();
              setReminders(reminders.filter(r => !r.completed));
            } catch (error) {
              Alert.alert('Error', 'Failed to clear completed reminders');
            }
          },
        },
      ]
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.inputContainer}>
        ...
        <TouchableOpacity
          style={[
            styles.addButton,
            (!newReminder.trim() || isAdding) && styles.addButtonDisabled,
          ]}
          onPress={handleAddReminder}
          disabled={!newReminder.trim() || isAdding}
        >
          {isAdding ? (
