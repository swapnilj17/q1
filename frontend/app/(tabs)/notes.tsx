...
export default function NotesScreen() {
  ...
  const loadNotes = async () => {
    ...
    try {
      let data = await db.getLocalNotes(user.id, { journal: false });
      if (searchQuery) {
  ...
  useFocusEffect(useCallback(() => { if (isDbReady) loadNotes(); }, [user, isDbReady, searchQuery]));
  ...
  const handleDeleteNote = (noteId: string) => {
    Alert.alert('Delete Note', 'Are you sure?', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        await db.deleteLocalNote(noteId);
  ...
  const styles = StyleSheet.create({
    ...
    searchContainer: {
      ...
      borderRadius: 12, marginHorizontal: 20, marginBottom: 16, paddingHorizontal: 16, height: 48,
    },
    ...
    noteTitle: { fontSize: 16, fontWeight: '600', color: colors.text, flex: 1 },
    pinIcon: { marginLeft: 8 },
    noteContent: { fontSize: 14, color: colors.textSecondary, lineHeight: 20, marginBottom: 12 },
    ...
    tag: { backgroundColor: colors.primaryLight, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6 },
  ...
  const renderNote = ({ item }: { item: Note }) => (
    ...
    >
      <View style={styles.noteHeader}>
        ...
        {item.pinned && <Ionicons name="pin" size={16} color={colors.accent} style={styles.pinIcon} />}
      </View>
      ...
      <View style={styles.noteFooter}>
        ...
        <Text style={styles.noteDate}>{format(new Date(item.updated_at || item.created_at), 'MMM d')}</Text>
      </View>
    </TouchableOpacity>
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}><Text style={styles.title}>Notes</Text></View>
      <View style={styles.searchContainer}>
        <Ionicons name="search" size={20} color={colors.textSecondary} />
        <TextInput style={styles.searchInput} placeholder="Search notes..." placeholderTextColor={colors.textSecondary}
      ...
      <FlatList data={notes} renderItem={renderNote} keyExtractor={(item) => item.id}
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>{searchQuery ? 'No notes found' : 'No notes yet'}</Text>
          </View>
        }
      />
      <TouchableOpacity style={styles.fab} onPress={() => router.push('/note/new')} activeOpacity={0.8}>
