...
export default function JournalScreen() {
  ...
  const loadJournalEntries = async () => {
    try {
      const entries = await api.getNotes({ journal: true });
      const marked: { [key: string]: any } = {};
      entries.forEach(entry => {
        if (entry.journal_date) {
          marked[entry.journal_date] = {
            marked: true,
  ...
  const loadEntryForDate = async (date: string) => {
    try {
      const entries = await api.getNotes({ journal_date: date });
      if (entries.length > 0) {
  ...
  const handleDateSelect = (day: DateData) => {
    ...
    loadEntryForDate(day.dateString);
  };
  ...
  const handleSaveEntry = async () => {
    ...
    try {
      if (journalEntry) {
        ...
        const updated = await api.updateNote(journalEntry.id, {
        ...
        setMarkedDates(prev => ({
          ...prev,
          [selectedDate]: { marked: true, dotColor: colors.primary },
        }));
      }
      setShowEditor(false);
